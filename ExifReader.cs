...
namespace MGL.Data.DataUtilities {
    ...
    ///     Test the ExifReader ...
    /// </summary>
    public static class ExifReaderTest {

        //--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        public static void Test() {
            string fileName = "C:/Dropbox/Dropbox/UNHCR RAHA/07_Meetings/2013-07-02 17.37.14.jpg";

            ExifReader ex = new ExifReader(fileName);

            double[] latArray = null;
            double[] longArray = null;
            double altitude;

            ex.GetTagValue<double[]>(ExifTags.GPSLatitude, out latArray);
            ex.GetTagValue<double[]>(ExifTags.GPSLongitude, out longArray);
            ex.GetTagValue<double>(ExifTags.GPSAltitude, out altitude);

            ConvertCoordinates cc = new ConvertCoordinates();
            double decimalLatitude, decimalLongitude;

            if (latArray != null) {
                decimalLatitude = cc.ConvertCoordinate(latArray[0], latArray[1], latArray[2]);
            }
            if (longArray != null) {
                decimalLongitude = cc.ConvertCoordinate(longArray[0], longArray[1], longArray[2]);
            }


            string make, model;
            ex.GetTagValue<string>(ExifTags.Make, out make);
            ex.GetTagValue<string>(ExifTags.Model, out model);

            DateTime dateTaken;
            ex.GetTagValue<DateTime>(ExifTags.DateTime, out dateTaken);

            uint width, height;
            ex.GetTagValue<uint>(ExifTags.ImageWidth, out width);
            ex.GetTagValue<uint>(ExifTags.ImageLength, out height);

            string name, type;
            ex.GetTagValue<string>(ExifTags.FileSource, out name);
            ex.GetTagValue<string>(ExifTags.SceneType, out type);

            double exposureTime, fNumber;
            ushort iso;
            ex.GetTagValue<double>(ExifTags.ExposureTime, out exposureTime);
            ex.GetTagValue<ushort>(ExifTags.ISOSpeedRatings, out iso);
            ex.GetTagValue<double>(ExifTags.FNumber, out fNumber);

            FileInfo fi = new FileInfo(fileName);
            long size = fi.Length;

            double sizeMB = Math.Round((double)size / 1048576.0, 3);
